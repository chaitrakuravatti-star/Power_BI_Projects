<h1> HR Attrition Analysis Dashboard</h1>


**Project Overview**


This Power BI project provides an in-depth HR Attrition Analysis to understand employee turnover patterns across departments, demographics, compensation, job levels, and work-life balance. The dashboard helps HR teams and business leaders identify key drivers of attrition and make data-driven workforce planning decisions.


**Objectives**

•	Analyze overall employee attrition and retention

•	Identify attrition trends by department, gender, age, salary, and job role

•	Understand the impact of business travel, work-life balance, and tenure on attrition

•	Compare attrition metrics across male vs female employees
________________________________________


**1️⃣ Overview – Attrition Summary**

Provides a high-level view of attrition across the organization.

**Visuals Included:**

•	Attrition by Department & Gender (R&D, Sales, HR)

•	Attrition by Business Travel & Gender

•	Attrition by Salary Hike Group & Gender

•	Attrition by Job Level & Work-Life Balance

•	Attrition by Gender & Marital Status

•	Job Satisfaction Rating by Job Role


**Key Insights:**

•	R&D department shows the highest attrition

•	Employees with lower salary hikes (11–15%) have higher attrition

•	Poor work-life balance strongly correlates with attrition

**2️⃣ Gender & Age Analysis**
Focuses on demographic-based attrition patterns.
**Visuals Included:**
•	Attrition by Age Group (Under 25, 25–34, 35–44, 45–54, Over 55)
•	Attrition by Gender
•	Attrition by Gender & Years at Company

**Key Insights:**

•	Highest attrition observed in 25–34 age group

•	Male attrition is higher than female across most tenure bands

•	Early-career employees (0–3 years) show maximum exits

**3️⃣ Job & Tenure Analysis**

Analyzes attrition across roles, experience, and satisfaction.

**Visuals Included:**

•	Attrition by Job Level & Work-Life Balance

•	Attrition by Department and Job Satisfaction Rating

•	Attrition by Years at Company

**Key Insights:**

•	Laboratory Technicians and Sales Executives have the highest attrition

•	Employees with low job satisfaction ratings leave more frequently

•	Attrition decreases significantly after long tenure (10+ years)
________________________________________

**Filters & Slicers**

Education Level (All, Technical, Life Sciences, Marketing, Medical, Others)

Over Time (Yes / No)

These slicers enable interactive analysis across all visuals.
________________________________________

**Tools & Technologies Used**

Power BI Desktop – Dashboard development & visualization

Power Query – Data cleaning and transformation

DAX – Measures and calculated columns

Dataset – HR Analytics employee dataset (anonymized)
________________________________________

**Business Use Cases**

Workforce retention strategy

HR policy optimization

Compensation & benefits planning

Talent management and succession planning

________________________________________

Author

Created by: Chaitra Kuravatti

LinkedIn: (https://www.linkedin.com/in/chaitra-kuravatti-aa918b143/)
